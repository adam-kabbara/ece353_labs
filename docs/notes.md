...
## Hashtable
Composed of key-value pairs, where each key is unique and maps to a specific value. The hashtable uses a hash function to compute an index into an array of buckets or slots, from which the desired value can be found.

The hash function takes an input (the key) and returns a fixed-size string of bytes. The output is typically a "hash code" that represents the input data. The hash code is then used to determine the index in the array where the corresponding value is stored, by taking the modulus of the hash code with the size of the array.

When two keys produce the same hash code (a collision), various collision resolution techniques can be employed, such as chaining (where each bucket contains a linked list of entries, like the attached image) or open addressing (where alternative slots are probed).

For more info you can review the ECE358 notes or check out [this resource](https://www.geeksforgeeks.org/hashing-data-structure/).
